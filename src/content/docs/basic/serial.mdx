...
Joskus, ohjelman toiminnan vaiheista olisi hyödyllistä saada tietoa silloin, kun ohjelma on toiminnassa. Arduinolla tämä on
...
Tyhjä `Serial.println()` funktio tulostaa tyhjän rivin. Tätä voi käyttää esimerkiksi, jos haluaa erotella `loop()` funktion ajojen
...
Tästä voidaan huomata, että `boolean` tyyppinen muuttuja tallentaa arvonsa itseasiassa numerona. Booleanilla on siis kaksi arvoa,
`true` (1) ja `false` (0).

## Harjoitus 2
Tässä harjoituksessa tulostetaan `boolean` tyyppisen muuttujan arvoa.
<LinkCard title="Harjoitus 2" href="https://wokwi.com/projects/413615269949071361" target="_blank" rel="noopener noreferrer" />
